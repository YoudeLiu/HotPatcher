...
IMPLEMENT_MODULE(FDefaultModuleImpl, PakFileUtilitiesEx);
...
bool ExecuteUnrealPakEx(const TCHAR* CmdLine)
{
	...
	if (FParse::Value(CmdLine, TEXT("-Batch="), BatchFileName))
	{
		...
		ParallelFor(Commands.Num(), [&Commands, &Result](int32 Idx) { if (!ExecuteUnrealPakEx(*Commands[Idx])) { Result = false; } });
		return Result;
	}
	...
	if(NonOptionArguments.Num() > 0)
	{
		...
		FString SourcePakFileName;
		FParse::Value(CmdLine, TEXT("-generatepatch="), SourcePakFileName);
		CmdLineParameters.SourcePatchPakFilename = SourcePakFileName;
